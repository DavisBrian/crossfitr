...
df <- readRDS(file = paste0(indir, "propel_17_1_results.rds"))

# Participation and scaling
df %>% 
  summarise(n    = sum(!is.na(df$time)),
            n_x  = sum(is.na(df$time))
            n_rx = 
            )
nrow(df)
sum(is.na(df$time))
...
#number completeing the workout
sum(df$time < ms("20:00"))
# Participation and scaling
...
rounds.rx <- tibble("x_min" = reps[seq(1,9,2)],
  ...
  labs(x="17.1 Reps",y="# of Athletes", title="Propel Crossfit Open 17.1 Scores") +
